...
const { model: newsTypeModel } = require('../new_type');
const sequelize = require('sequelize');

async function init() {
    let creatrType1 = await newsTypeModel.build({
        title: '科技新闻'
    }).save();

    let creatrType2 = await newsTypeModel.build({
        title: '互联网新闻'
    }).save();

    let result = await newsModel.build({
        title: '华为正开发自主操作系统以备不测 你认为有必要吗', description: '谈及移动操作系统，全世界目前整个手机领域只有两种主要的选择：iOS 和 Android，这两大操作系统的全球市场份额总计达 99%。虽然在过去十年时间里，也有一些其他操作系统可以选择，例如微软的 Windows Mobile 和 Windows Phone、诺基亚的 Symbian 和 MeeGo，三星的 Tizen 以及黑莓 BlackBerry OS，但是在 2018 年，这些所谓的“其他”系统，大多数要么已经被搁置了，要么根本已经不存在了。',
